...
export interface User {
  no: string; // Serial number or unique identifier
  name: string;
  mobile: string;
  email: string;
  role: string; // e.g., "Admin", "Manager", "Staff"
  store: string; // Associated store/branch
  status: boolean; // true for active, false for inactive
  createdBy: string;
  createdAt: string;
  billType: string; // e.g., "GST", "Non-GST"
}
...
export interface NewUserFormData {
  ...
  mobile: string;
  email: string;
  password?: string; // Optional for edit scenarios, required for new user creation
  role: string;
  billType: string;
}
