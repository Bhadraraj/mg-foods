...
interface BillReturnModalProps {
  isOpen: boolean;
  onClose: () => void;
  billData: any; // Consider defining a more specific type for billData
}
...
const BillReturnModal: React.FC<BillReturnModalProps> = ({ isOpen, onClose, billData }) => {
  ...
  useEffect(() => {
    if (isOpen && billData && billData.products) {
      // Initialize return products with a 'selectedForReturn' flag and return amount
      // Ensure 'id' is unique for each product, falling back to index if not present
      const initialReturnProducts = billData.products.map((p: any, index: number) => ({
        ...p,
        id: p.id || index,
        selectedForReturn: false,
        returnAmount: 0, // Initialize return amount for each product
      }));
      setReturnProducts(initialReturnProducts);
      setTotalReturnAmount(0); // Reset total return amount on modal open
    }
  }, [isOpen, billData]);
  ...
  const handleProductReturnToggle = (productId: number) => {
    setReturnProducts(prevProducts => {
      ...
      // Recalculate total return amount based on selected products and their returnAmount
  ...
  const handleReturnAmountChange = (productId: number, value: string) => {
    setReturnProducts(prevProducts => {
      ...
      // Recalculate total return amount based on entered returnAmount for selected products
      const newTotal = updatedProducts.reduce((sum, p) =>
  ...
  const handleReturn = () => {
    ...
    // Implement actual return logic here (e.g., API call to backend)
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center p-4 z-50 font-sans">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-6xl relative overflow-hidden">
        ...
        {billData ? (
          <div className="p-6 overflow-y-auto max-h-[80vh]"> {/* This max-h is for the entire modal content */}
            {/* Bill, Customer, Payment Details */}
            ...
            <div className="border border-gray-200 rounded-lg overflow-hidden mb-6 shadow-sm">
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200 **max-h-24 overflow-y-auto block**"> 
                    {returnProducts.map((product, index) => (
                      <tr key={product.id} className="hover:bg-gray-50 **table-row**"> {/* Ensure rows are table-row display */}
