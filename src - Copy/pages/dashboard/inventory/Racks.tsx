...
// Assuming RackDropdown exists and is imported correctly.
// A placeholder is used here for completeness.
...
// Define interfaces (consistent with parent component)
interface ItemRack {
  rack: string;
  currentStock: number;
  minStock: number;
...
const Racks: React.FC<RacksProps> = ({
  ...
  // State for the new stock transfer feature
  ...
  // Update modal item when inventoryItems change from parent
  useEffect(() => {
  ...
  const openItemRacksModal = (item: InventoryItem) => {
    ...
    // Reset transfer form state when opening modal
    setTransferQty(0);
    setTransferFrom('');
    setTransferTo('');
    setTransferError('');
  ...
  // Helper function to update modal item's rack details directly (for inputs)
  const updateModalItemRack = (rackName: string, field: keyof ItemRack, value: number | string) => {
    ...
    // NEW LOGIC: Remove rack if currentStock becomes 0
  ...
  // Logic for the new "Transfer Stock" feature
  const handleStockTransfer = () => {
    if (!itemRacksModalItem) return;
    setTransferError(''); // Reset error message
    ...
    // Decrease stock from source rack
    ...
    if (toRackExists) {
        // Increase stock in destination rack
        updatedRacks = updatedRacks.map(r =>
            r.rack === transferTo
            ? {...r, currentStock: r.currentStock + transferQty}
            : r
        ...
        // Add new rack to item if it doesn't exist
    ...
    // NEW LOGIC: Remove source rack if its stock becomes 0 after transfer
    updatedRacks = updatedRacks.filter(r => !(r.rack === transferFrom && r.currentStock === 0));


    // Update state locally and in parent
  ...
  // Handle the submission of the modal (e.g., when the user clicks 'Submit')
  const handleSubmitModal = () => {
    // You can add any final validation or data processing here before closing
  ...
  return (
    <>
      ...
      {/* Item Racks Management Modal - MODIFIED */}
      {itemRacksModalItem && (
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-col md:flex-row gap-8 w-full mt-4">
                      ...
                      {itemRacksModalItem.racks.map((rackDetail) => (
                        <div key={rackDetail.rack} className="flex items-center gap-3 border-b pb-3 flex-wrap">
                          ...
                          {/* <div className="flex items-center gap-2">
                            ...
                            <input
                              type="number"
                              value={rackDetail.minStock}
                              onChange={(e) => updateModalItemRack(rackDetail.rack, 'minStock', e.target.value)}
                              className="w-20 px-2 py-1 border rounded"
                            />
                          </div> */}
                          {/* <div className="flex items-center gap-2">
                            ...
                            <input
                              type="number"
                              value={rackDetail.maxStock}
                              onChange={(e) => updateModalItemRack(rackDetail.rack, 'maxStock', e.target.value)}
                              className="w-20 px-2 py-1 border rounded"
                            />
                          </div> */}
                          <button
              ...
              <div className="flex justify-end gap-3 mt-6 pt-4 border-t">
                  <button
                    onClick={handleSubmitModal} // New submit handler
